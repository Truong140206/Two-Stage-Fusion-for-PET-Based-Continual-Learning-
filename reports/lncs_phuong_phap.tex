...
\node[tinylabel] at (5.76,5.78) {Max / tác vụ $\rightarrow z$\\Trộn $w$\\Argmax};
...
hiện tại. (b) Cùng điểm RP cố định $s$ sửa đề xuất tác vụ DRM và hỗ trợ hợp
nhất lớp có cổng biên. (c) Dữ liệu tác vụ hiện tại cập nhật
$G,C,W_{\rm out}$ bằng ridge dạng đóng, không phát lại dữ liệu hay dùng
gradient. Ảnh: T.~G.~Barnes/USFWS (phạm vi công cộng).}
\label{fig:pipeline}
\end{figure}
