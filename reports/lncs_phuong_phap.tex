...
%
\begin{abstract}
Các hệ học liên tục hiệu quả tham số dùng bộ điều hợp riêng cho từng tác vụ
phải lựa chọn bộ điều hợp khi không biết định danh tác vụ, trong khi định tuyến
tốt hơn chưa chắc dẫn tới phân lớp tốt hơn. Chúng tôi đề xuất hợp nhất điểm số
hai tầng bằng một đầu chiếu ngẫu nhiên (RP), được khớp bằng hồi quy ridge trên
đặc trưng LoRA cố định của tác vụ đầu tiên. Các thống kê của đầu này được cộng
dồn qua các tác vụ mà không lưu mẫu cũ hay thay đổi việc huấn luyện bộ điều hợp
theo tác vụ. Cùng một điểm RP được dùng tại hai điểm quyết định: hợp nhất đề
xuất tác vụ trước khi đối sánh lại và hợp nhất lớp có cổng biên sau đó.
Trên ba bộ chuẩn và bốn seed, phương pháp tăng độ chính xác đề xuất DRM
$1.10$--$2.26$ điểm phần trăm và độ chính xác phân lớp $0.69$--$1.50$ điểm.
Một lưới một seed trên bốn bộ dữ liệu và năm thiết lập tiền huấn luyện cho thấy
độ chính xác tăng ở cả hai mươi phép so sánh. Các đối chứng cùng đặc trưng và
ablation có điều kiện tách đóng góp của bộ phân lớp RP khỏi tác động của định
tuyến và cổng. Các tác động phụ thuộc bộ dữ liệu và cấu hình hợp nhất; không
thành phần nào luôn có lợi. Kết quả ủng hộ cải thiện dự đoán, nhưng không cho
thấy sự giảm nhất quán của quên hay suy giảm chuyển giao ngược.
...
\node[heading,text=io] at (0,9.30) {(a) Học các mô-đun PET trên tác vụ $t$};
\node[tinylabel,anchor=east,text=black!65] at (12.1,9.30)
  {F: cố định \quad T: huấn luyện \quad A: cập nhật giải tích};
...
\pic at (1.95,6.11) {vit={ib/ViT trần + $g_\omega$}};
...
\node[tinylabel,text=io] at (5.76,6.57) {\textbf{1 Định tuyến}};
\draw[io!25] (5.10,6.36) -- (6.42,6.36);
\node[tinylabel] at (5.76,5.78) {Max / tác vụ\\Điểm $z$\\Trộn $w$\\Argmax};
\draw[arr,draw=io] (6.55,6.15) -- node[tinylabel,above] {$\hat t_{\rm DRM}$} (7.56,6.15);
\pic at (8.03,6.11) {vit={ib/DRM / CRM + $g$}};
\node[tinylabel,text=ib] at (8.03,7.00) {Các lượt điều hợp\\có điều kiện};
...
\node[tinylabel,text=ir] at (10.105,6.57) {\textbf{2 Phân lớp}};
\draw[ir!25] (9.31,6.36) -- (10.90,6.36);
\node[tinylabel] at (10.105,5.78)
  {Cổng từ $\ell$\\Điểm $z$\\Trộn $\beta_i$\\Đổi thang};
...
\node[heading,text=it] at (.20,4.82) {Nhánh RP cố định};
\node[tinylabel,text=it,anchor=west] at (2.98,4.82) {$p_1$ đóng băng; thêm một lượt};
\draw[arr,draw=it] (input.south) -- (.52,5.18) -- (.07,5.18) --
  (.07,3.89) -- (1.48,3.89);
\pic at (1.95,3.85) {vit={it/$p_1$ cố định}};
...
\node[tinylabel,text=ip] at (8.255,3.12) {Điểm lớp $s$};
...
\node[tinylabel,text=ip,fill=white,inner sep=2pt] at (7.14,4.48) {tái dùng cùng $s$};
...
\node[heading,text=it] at (0,2.60) {(c) Khớp RP trên tác vụ hiện tại: chỉ cập nhật thống kê và trọng số đầu ra};
...
\pic[scale=.80] at (1.91,1.42) {vit={it/$p_1$ cố định}};
...
\draw[arr,draw=it,dashed] (.57,.62) -- (.57,.29) --
  node[tinylabel,below,pos=.53] {nhãn quan sát $y$ (chỉ huấn luyện)}
  (5.57,.29) -- (cross.south);
\node[tinylabel,draw=ip!65,fill=ip!5,rounded corners=2pt,inner sep=5pt]
  (ridge) at (8.65,1.44) {\textbf{A} Giải ridge\\$(G+\lambda I)W_{\rm out}=C$};
\draw[arr,draw=ip] (gram.east) -- (6.99,1.83) |- (ridge.west);
\draw[arr,draw=ip] (cross.east) -- (6.99,1.06) |- (ridge.west);
\node[tinylabel,text=black!65] at (8.65,.70) {Không cập nhật gradient};
...
\node[tinylabel,text=ip] at (10.84,2.14) {dùng trong (b)};
\end{tikzpicture}
\caption{Vòng đời tham số và hợp nhất hai tầng. (a) Quy trình PET kế thừa
huấn luyện bộ điều hợp/các đầu, không huấn luyện mạng xương sống. (b) Mọi
mô-đun đều cố định: Tầng 1 sửa đề xuất DRM; Tầng 2 đặt cổng cho hợp nhất lớp
bằng $\ell$. Một vector điểm RP phục vụ cả hai tầng. (c) Dữ liệu huấn luyện
hiện tại cập nhật giải tích $G,C,W_{\rm out}$ với $p_1,W$ cố định. Chỉ các
lớp/tác vụ đã thấy tham gia. Ảnh và cột chỉ minh họa, không phải dự đoán đo
được. Ảnh: T.~G.~Barnes/USFWS (phạm vi công cộng).}
\label{fig:pipeline}
\end{figure}
...
$u$. Cả $r$ và điểm RP $s$ được ánh xạ về tác vụ đã thấy bằng
$r^{\mathrm{task}}_t = \max_{c \in \mathcal{C}_t} r_c$ and
$s^{\mathrm{task}}_t = \max_{c \in \mathcal{C}_t} s_c$ for
$t=1,\dots,\hat{T}$, trong đó $\hat{T}$ là số tác vụ đã thấy; lớp thuộc tác
...
Acc@task trong bộ đánh giá phát hành đo đề xuất tác vụ DRM. Ở đường định tuyến
theo lớp mặc định, CRM có thể đổi logit đầu ra mà không cập nhật định danh tác
vụ chuyển vào metric này; vì thế đây không phải phép đo lựa chọn bộ điều hợp
cuối sau CRM. Acc@1, Acc@5 và Loss dùng logit phân lớp cuối. Metric cuối lấy
...
Ví dụ, hai đường cùng cho ImageNet-R seed 42 Acc@1 bằng $74.0191$. Đồng nhất
...
\caption{Kết quả chính: Acc@1 cuối trung bình theo tác vụ (\%). Các nhóm Sup-21K
...
Dấu gạch nghĩa là không đánh giá/báo cáo. IMR/IMA là ImageNet-R/A; CIFAR/CUB
là CIFAR-100/CUB-200. Chữ đậm chỉ so sánh hai phương pháp được ghép trong từng
nhóm giữa, không so mọi protocol.}
\label{tab:main}
\begin{tabular}{@{}lrrrrr@{}}
\toprule
Phương pháp & IMR & CIFAR & CUB & IMA & 5-Datasets\\
\midrule
\multicolumn{6}{@{}l}{\emph{Bối cảnh Sup-21K đã công bố: trung bình ba seed từ~\cite{ref_hrmpet}}}\\
...
\midrule
\multicolumn{6}{@{}l}{\emph{Các lượt Sup-21K được ghép của chúng tôi: seed 42--45}}\\
HRM-PET & $73.94\pm0.48$ & $89.78\pm0.03$ & $86.38\pm0.25$ & -- & --\\
Đầy đủ (chúng tôi) & $\mathbf{75.18\pm0.24}$ & $\mathbf{90.47\pm0.05}$ & $\mathbf{87.88\pm0.27}$ & -- & --\\
\midrule
\multicolumn{6}{@{}l}{\emph{Lưới Sup-21K của chúng tôi: seed 42 (trùng nhóm bốn seed)}}\\
HRM-PET & 74.02 & 89.77 & -- & 45.85 & 91.37\\
Đầy đủ (chúng tôi) & \textbf{75.51} & \textbf{90.53} & -- & \textbf{47.42} & \textbf{93.77}\\
\midrule
\multicolumn{6}{@{}l}{\emph{AugReg-21K$\rightarrow$1K thăm dò: thứ tự 1993, seed huấn luyện 1}}\\
...
Độ chính xác đề xuất DRM tăng $1.10$--$2.26$ điểm và Acc@1 tăng
$0.69$--$1.50$ điểm, với khoảng ghép cặp dương trên cả ba bộ dữ liệu. Acc@5
cũng tăng $0.22$--$1.31$ điểm với khoảng dương. Mức tăng Acc@1 lặp lại ở mọi
...
Cả hai mươi ô tăng Acc@1 từ $+0.59$ đến $+3.58$ điểm, trên đường cơ sở Acc@1
$31.8$--$93.8$; mức tăng Acc@task là $+0.97$ đến $+4.57$.

\begin{table}[htbp]
\centering
\small
\setlength{\tabcolsep}{6pt}
\caption{Chênh lệch Acc@1 (điểm phần trăm), phương pháp đầy đủ trừ HRM-PET
được ghép, seed $42$. Mọi ô dùng cùng $w=0.7$, $\beta=0.5$ và cổng biên;
đây là so sánh một seed, không phải khoảng tin cậy.}
\label{tab:grid}
\begin{tabular}{lrrrr}
\toprule
...
Độ chính xác cuối trên các tác vụ cũ tăng ở mọi ô (trung bình $+1.83$), so với
$+1.58$ tại thời điểm học. Backward, tức hiệu giữa hai đại lượng, xấu đi ở năm
ô vì độ chính xác lúc học tăng nhiều hơn, không phải độ chính xác cuối giảm.
Phân rã này dùng log theo tác vụ đã làm tròn và chỉ mang tính mô tả.
...
\caption{So sánh thành phần: Acc@1 cuối (\%), trung bình $\pm$ SD mẫu trên
seed 42--45. Phương pháp đầy đủ dùng $w=0.7$, $\beta=0.5$ và cổng biên.
Chữ đậm đánh dấu trung bình cao nhất mỗi cột, không phải tuyên bố ý nghĩa
thống kê. Phương pháp đầy đủ không luôn tốt nhất trong các biến thể.}
...
Ablation năm mạng xương sống trên ImageNet-R dùng seed $42$; Acc@1 cơ sở từ
$68.47$ đến $74.02$. Chúng tôi báo cáo chênh lệch giữa các biến thể đã chỉ định.
Biến thể chỉ định tuyến và chỉ lớp không cổng tăng Acc@1 trên từng mạng lần
lượt $+0.29$--$+0.73$ và $+0.51$--$+3.07$. Thêm định tuyến vào hợp nhất lớp
không cổng làm Acc@1 đổi $-0.07$ đến $+0.02$, chưa chứng minh tương đương.

Acc@1 của đầy đủ trừ chỉ lớp có cổng là $+0.13$, $+0.29$, $+0.78$, $+0.43$ và
...
đầy đủ không cổng là $-0.26$ Acc@1 trên MoCo-v3 và $-0.54$ trên iBOT-1K.
...
nhất thứ nhất làm thay đổi Acc@task.
...
\begin{axis}[
  ...
  ylabel={Acc@1 cuối (\%)},font=\footnotesize,
...
được báo cáo. Với $\beta=0.5$, Acc@1 chỉ trải $0.11$ điểm, so với khoảng một
điểm của Acc@task. Trên bốn seed, $0.6$ tăng Acc@task
$+0.332$ $[+0.143,+0.522]$ và Acc@1 $+0.108$ $[-0.064,+0.280]$ so với $0.7$
khi dùng cổng và $\beta=0.5$; dấu của chênh lệch Acc@1 đảo ở seed 43. Vì vậy,
...
lớp tăng độ chính xác phân lớp $0.69$--$1.50$ điểm Acc@1 so với hệ PET được
