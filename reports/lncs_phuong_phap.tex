...
LoRA hạng $8$ sửa các phép chiếu key/value trong năm block đầu. Khảo sát bốn seed bao gồm ImageNet-R~\cite{ref_imr}, CIFAR-100~\cite{ref_cifar},
CUB-200-2011~\cite{ref_cub}, ImageNet-A~\cite{ref_ima} và protocol năm bộ dữ
liệu của~\cite{ref_hrmpet}. Các protocol tương ứng dùng $10\times20$,
$10\times10$, $10\times20$, $10\times20$ lớp và mỗi miền làm một tác vụ. Lưới
rộng hơn bỏ CUB và thử Sup-21K, MoCo-v3~\cite{ref_moco},
iBOT-1K/iBOT-21K~\cite{ref_ibot} cùng DINO~\cite{ref_dino}.
...
\midrule
\multicolumn{5}{@{}l}{\emph{Các lượt Sup-21K được ghép: seed 42--45}}\\
...
cũng tăng $0.22$--$1.31$ điểm với khoảng dương. Trên ba bộ dữ liệu này, mức
tăng Top-1 lặp lại ở mọi seed, nhưng các mức tăng tổng hợp không đếm bao nhiêu
sửa đổi định tuyến thực sự sửa được dự đoán lớp. Cả sáu khoảng ghép cặp của
metric ghi nhớ đều chứa không: chưa chứng minh được lợi ích nhất quán hay tính
tương đương. Ví dụ, Forgetting trên CIFAR đổi $-0.150$ $[-0.302,+0.002]$ và
Backward đổi $+0.136$ $[-0.015,+0.287]$. Không có biên tương đương được định trước.

Với IMA, chênh lệch Top-1 là $+0.82$ và khoảng $95\%$ là
$[-0.55,+2.18]$ (một seed âm). Với 5-Datasets, các giá trị tương ứng là
$+2.00$ và $[+1.51,+2.49]$ (cả bốn seed dương). Chỉ khoảng IMA cắt qua không;
Hình~\ref{fig:paired} không gồm metric định tuyến hay ghi nhớ của hai bộ này.
...
năm mạng xương sống chỉ dùng một seed. Top-1 được ghép dùng bốn seed trên năm
bộ dữ liệu; các khoảng định tuyến và ghi nhớ trong Hình~\ref{fig:paired} bao phủ
ba bộ. Tác động định tuyến có điều kiện trên CIFAR chưa rõ, và lợi ích của cổng
thay đổi theo mạng xương sống.
...
Trên năm bộ dữ liệu và bốn seed, tái dùng một bộ dự đoán RP có đặc trưng cố
định cho đề xuất tác vụ và điểm lớp làm Top-1 trung bình tăng $0.69$--$2.00$
điểm; khoảng ImageNet-A cắt qua không. Không huấn luyện lại bộ điều hợp hay lưu
ảnh cũ. So sánh thành phần cho thấy hợp nhất lớp tạo phần lớn mức tăng;
