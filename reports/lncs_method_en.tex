...
% Colour carries meaning in Fig. 1 and is used for nothing else: green for the
% frozen backbone, blue for a trained classifier, orange for the LoRA pool,
% purple for the random projection, and red for the two blocks this paper adds.
\definecolor{cfroz}{RGB}{198,224,180}
\definecolor{cmlp} {RGB}{189,215,238}
\definecolor{cpool}{RGB}{248,203,173}
\definecolor{crp}  {RGB}{214,196,232}
\definecolor{cours}{RGB}{192,0,0}
\definecolor{cpanel}{RGB}{240,246,252}
% A padlock, drawn rather than taken from a symbol font, so it needs no package
% and scales with the surrounding text. It marks the blocks that stay frozen.
\newcommand{\lk}{\tikz[baseline=-0.45ex]{
  \draw[line width=0.3pt, black!75] (0,0.85mm) arc (180:0:0.5mm);
  \fill[black!75, rounded corners=0.2mm]
       (-0.85mm,-0.85mm) rectangle (0.85mm,0.85mm);}}
%
\begin{document}
%
...
one module. We show that a second source is already present inside the trained
model and can be read out without gradient training: a second-order head built
on a frozen random projection of the existing LoRA features, its weights the
closed-form ridge solution of statistics accumulated one task at a time. It is
fused at two points, both at inference. The first stage blends it with the
routing signal after standardising the two onto a common scale; the second uses
it as a secondary classifier, weighted per sample in inverse proportion to how
decided the primary classifier already is, so the blend opens only on contested
samples. Across three benchmarks and four seeds, routing accuracy rises by
$+1.10$ to $+2.26$ points and classification accuracy by $+0.69$ to $+1.50$,
each several times the seed-to-seed standard deviation, while the retention
metrics show no change we can resolve. An ablation over four seeds separates the
stages: the second carries most of the gain, and the first adds a further
$+0.69$ and $+0.33$ on two benchmarks and nothing measurable on the third.
...
\item We show that a second-order head over a frozen random projection of the
      ...
      supplies a second source of evidence whose errors are decorrelated from
      the first, and we fuse the two at the routing and the classification
      level alike, entirely at inference time.
...
Three score vectors recur below, each over the classes seen so far: $u$ from
TII's auxiliary classifier $g_\omega$, $\ell$ from the shared head $g_{\theta_g}$
once an adapter is attached, and $s$ from the projection head of
Sect.~\ref{sec:method}. We write $\mathcal{T}(\cdot)$ for the map from a class to
its task, $\hat{t}_f$ and $\hat{t}_s$ for the task identity before and after
re-matching, and $z(\cdot)$ for per-sample standardisation, using the population
standard deviation floored at $10^{-6}$.
...
At seed 42 on ImageNet-R, TII routes $63.80\%$ of samples correctly, HRM-PET
after DRM and CRM $77.79\%$, and the head on its own $77.12\%$ --- but the
\emph{union} of TII and the head, the fraction at least one of them routes
correctly, reaches $82.99\%$; the corresponding unions are $92.86\%$ on
CIFAR-100 and $96.26\%$ on CUB-200.

The two routers therefore fail on \emph{different} samples. The union exceeds
TII by $19.19$ points of the whole ImageNet-R test set, and since TII is wrong
on $36.20\%$ of it, those points are $53.0\%$ of TII's errors repaired by the
head; the same figures are $62.0\%$ on CIFAR-100 and $46.9\%$ on CUB-200. That
complementarity is the necessary condition for fusion to mean anything --- had
the two failed together, their union could not exceed the stronger of them, and
it exceeds both by more than five points. The union is an oracle: it uses the
...
The RP head, however, is not merely a router: it is a complete classifier the
routing stage discards, and the same complementarity holds at that level. On
ImageNet-R the routed head classifies $74.31\%$ correctly and the projection
head $70.08\%$, yet their union reaches $78.89\%$: even four points weaker
overall, the head is right on $4.58\%$ of the samples the primary head cannot
handle, and on $2.43\%$ and $3.87\%$ for CIFAR-100 and CUB-200. These two
measurements lead directly to the two components of Sect.~\ref{sec:method}: the
first fuses at the routing level, the second uses the same head as a second
classifier.
...
\subsubsection{Classifiers fitted without gradient descent.}
...
cannot forget --- reaches $72.0\%$~\cite{ref_ranpac}. Such a head is therefore
not a weak fallback, and its gap to a routed pipeline is not explained by
forgetting; what it gives up is the structure routing provides. We take the head
of~\cite{ref_ranpac} into a routing pipeline as a second source of evidence, not
...
\begin{figure}[!ht]
\centering
% No \resizebox: the picture is laid out to fit \textwidth at its natural size,
% so the labels keep the body font. The previous version was wider than the
% column and was scaled down, which is why its text came out below \tiny.
\begin{tikzpicture}[
  font=\scriptsize,
  >={Stealth[length=1.5mm]},
  blk/.style={draw=black!55, rounded corners=1.5pt, minimum height=6.5mm,
              inner xsep=2.5pt, align=center, font=\scriptsize},
  froz/.style={blk, fill=cfroz},
  mlp/.style={blk, fill=cmlp},
  pool/.style={blk, fill=cpool},
  rph/.style={blk, fill=crp},
  io/.style={blk, fill=black!5, draw=black!70},
  ours/.style={blk, draw=cours, line width=0.9pt, fill=cours!7,
               minimum height=8mm},
  lab/.style={font=\scriptsize\itshape, text=black!55, inner sep=1pt},
  tag/.style={font=\tiny, text=black!70, inner sep=1.2pt, fill=white}
]

% ---------------- row 1 : the two sources ----------------
\node[io] (in) at (0,0) {test\\image};
\node[froz, anchor=west] (bb)   at (1.3,0.62) {\lk\,$h_{\mathrm{ptm}}$};
\node[mlp,  anchor=west] (gw)   at ([xshift=3mm]bb.east) {\lk\,$g_\omega$};
\node[pool, anchor=west] (lo1)  at (1.3,-0.62) {\lk\,LoRA $p_1$};
\node[rph,  anchor=west] (proj) at ([xshift=3mm]lo1.east) {\lk\,$W$, ReLU};
\node[rph,  anchor=west] (ridge)at ([xshift=3mm]proj.east) {ridge $W_{\mathrm{out}}$};

\begin{scope}[on background layer]
\node[draw=black!40, dashed, rounded corners=2pt, fill=cpanel,
      fit=(bb)(gw)(lo1)(proj)(ridge), inner sep=3mm] (panel) {};
\end{scope}
\node[lab, anchor=south west] at ([xshift=0.5mm]panel.north west)
      {two sources of evidence, one forward pass each};

\node[ours, anchor=west] (f1) at ([xshift=5mm]panel.east)
      {\textcolor{cours}{\textbf{Stage 1}}\\routing fusion};

\draw[->] (in.east) -- ++(0.15,0) |- (bb.west);
\draw[->] (in.east) -- ++(0.15,0) |- (lo1.west);
\draw[->] (bb) -- (gw);
\draw[->] (lo1) -- (proj);
\draw[->] (proj) -- (ridge);
\draw[->] (gw.east)    -- ++(0.30,0) |- (f1.west);
\draw[->] (ridge.east) -- ++(0.30,0) |- (f1.west);
\node[tag] at ([xshift=2.1mm, yshift=1.3mm]gw.east) {$u$};
\node[tag] at ([xshift=2.1mm, yshift=1.3mm]ridge.east) {$s$};

% ---------------- row 2 : the routed pipeline ----------------
\def\ry{-2.95}
\node[pool, minimum width=1.85cm, minimum height=9.5mm] (pl) at (1.0,\ry) {};
\foreach \i in {0,...,3}{
  \node[draw=black!55, fill=cpool!55, minimum width=2.3mm, minimum height=3.2mm,
        inner sep=0] at ([xshift=(\i-1.5)*3.1mm, yshift=1.9mm]pl.center) {};}
\node[font=\tiny, anchor=center] at ([yshift=-2.4mm]pl.center) {param.\ pool $P$};

\node[blk, anchor=west] (dr) at ([xshift=4.5mm]pl.east) {DRM\,/\,CRM};
\node[mlp, anchor=west] (sh) at ([xshift=4.5mm]dr.east) {shared head $g_{\theta_g}$};
\node[ours, anchor=west] (f2) at ([xshift=4.5mm]sh.east)
      {\textcolor{cours}{\textbf{Stage 2}}\\gated fusion};
\node[io, anchor=west] (out) at ([xshift=4.5mm]f2.east) {prediction};

\draw[->] (pl) -- (dr);
\draw[->] (dr) -- (sh);
\draw[->] (sh) -- (f2);
\draw[->] (f2) -- (out);
\node[tag] at ($(sh.east)!0.5!(f2.west)$) {$\ell$};

% ---------------- the two links between the rows ----------------
% Both run in the empty lane below the panel, so they cross no block.
\draw[->] (f1.south) -- ++(0,-1.28) -| (pl.north);
\node[tag] at ([xshift=-1.1cm, yshift=-1.28cm]f1.south) {$\hat{t}_f$};
\draw[->, dashed, cours!80] (ridge.south) -- ++(0,-1.2) -| (f2.north);
\node[tag, text=cours!85] at ([xshift=2mm, yshift=-1.2cm]ridge.south) {$s$};

\end{tikzpicture}
\caption{The inference pipeline. Red marks the two blocks this paper adds;
everything else is HRM-PET. Padlocks mark what stays frozen: the backbone, the
first task's adapter and the random projection are never updated, and the ridge
head is refitted in closed form once per task. The head is computed once per
sample and read twice --- its task-level scores $s$ join TII's $u$ at Stage 1,
and the same vector, read over classes, joins the routed logits $\ell$ at
Stage 2 (red dashed).}
\label{fig:pipeline}
\end{figure}
...
The second source must satisfy two constraints. Nothing in it may be updated by
gradient descent across the sequence, since parameters trained that way can be
overwritten by later tasks --- which does not forbid it from being fitted, only
from being fitted in a way that forgets. And it must preserve the exemplar-free
protocol. A second-order classification head in the style of
RanPAC~\cite{ref_ranpac} satisfies both.
...
classes overlap so much that no hyperplane separates them, while after the
expansion a linear classifier is expressive enough. The nonlinearity is
mandatory --- without it $fW$ is a linear transform of $f$ of rank at most $d$,
and projecting into $M \gg d$ dimensions adds no separating power at all.
...
\label{eq:ridge}
\end{equation}
No separate dimensionality-reduction step is needed: $W_{\mathrm{out}}$ collapses
$M$ dimensions to $|\mathcal{Y}|$ in one product, so the trajectory is
$768 \to 10^4 \to |\mathcal{Y}|$ and the expansion exists \emph{in order to make}
the linear step down work. The Gram inverse is what makes it work: the
post-nonlinearity dimensions are strongly correlated, and
$(G + \lambda I)^{-1}$ decorrelates them.
...
\emph{population} standard deviation and floors the divisor at $10^{-6}$. At the
first stage $\hat{T}=1$, where the Bessel-corrected estimator divides by zero and
returns a NaN no floor repairs, while the population estimator returns $0$, which
it does. Later stages differ by a constant $\sqrt{\hat{T}/(\hat{T}-1)}$ shared by
both sources, so it cancels in the arg max.

The identity $\hat{t}_f$ replaces the output of (\ref{eq:tii}) and is handed to
DRM and CRM. We replace \emph{exactly one} module, the first matching step;
every downstream mechanism is untouched. At $w = 1.0$ the fused ordering is that
of $u^{\mathrm{task}}$, so this stage alone reduces to the baseline's first
matching step --- though recovering the baseline needs $\beta = 0$ as well,
since the second stage has its own weight.
...
rank $8$. The four-seed study uses Split-ImageNet-R, Split-CIFAR100 and
Split-CUB200 in $10$ tasks each; the grid of Sect.~\ref{sec:grid} replaces
...
checkpoints through the same code path, so a reported difference carries no
training noise. The projection head uses $M = 10^4$ dimensions, a ReLU
non-linearity and $\lambda = 10^4$; no temperature scaling is applied.

Writing $a_{i,t}$ for the accuracy on task $i$ at stage $t$, we report routing
accuracy (Acc@task), classification accuracy at ranks one and five (Acc@1,
Acc@5), cross-entropy Loss, and the two retention measures
\begin{equation}
...
\label{eq:metrics}
\end{equation}
Both averages run over the $T-1$ tasks preceding the last, which contributes an
identically zero term to each: evaluated at one stage only, its maximum and its
final accuracy are the same entry. Dividing by $T$ instead would report every
retention figure a factor $(T-1)/T$ --- here $10\%$ --- too small. The maximum
runs over the stages at which task $i$ was actually evaluated, $t \ge i$. Lower
is better for Loss and Forgetting, higher for the rest; both retention measures
compare a task against \emph{itself} at an earlier stage rather than against
another method, which is why they behave differently from the accuracy axes.

\subsubsection{Sanity check.}
$w = 1.0$ in (\ref{eq:route}) puts the whole routing weight on TII and
...
task reached when it was learned, so it charges a method for learning better.
...
the five cells where it worsens are cells that learned more, not cells that
forgot more.
...
That redundancy is conditional, and reporting only the ungated pair would make
it look absolute. With the gate on, routing is worth $+0.13$ to $+0.77$ and is
positive on all five backbones; the dependence runs the other way too, the gate
costing $-1.32$ to $+0.85$ when the classifier is alone against $-0.54$ to
$+0.96$ once routing is present. Each repairs what the other breaks: the gate
withholds the projection head where the routed head is confident, and a wrong
route is one of the things that makes it confident for the wrong reason.
...
method against its second stage alone --- over four seeds.
...
interval excludes zero on ImageNet-R and CIFAR-100, and the difference is
positive on every individual seed of both. On CUB-200 it does not --- $-0.13$
with an interval spanning zero --- so there the classification stage alone is as
good as the full method and possibly better, and we report that rather than
average it away. CIFAR-100 carries the most weight, being the one benchmark here
on which no hyper-parameter was chosen: had the effect appeared only on
ImageNet-R, where $w$, $\beta$ and the gate were all selected, it could not be
separated from selection bias.

The first stage is also the only component that moves Acc@task, by $+2.18$ to
$+3.95$ across the grid of Sect.~\ref{sec:grid}, an axis a routing method
reports in its own right. But the two Acc@1 contributions must not be added.
Note also that the classifier's isolated contribution is smallest by far on
Sup-21K, $+0.51$ against $+2.23$ to $+3.07$ on the other four --- and Sup-21K is
the backbone every design decision was taken on.

\section{Limitations}
\label{sec:limits}

Hyper-parameters were selected on ImageNet-R, so that dataset does not
contribute independent evidence; CIFAR-100 and CUB-200 do, and the first-stage
result of Table~\ref{tab:classonly} rests on CIFAR-100 for exactly that reason.
The four-seed protocol covers one backbone, and the twenty-cell grid covers one
seed; no cell has both, and the single-seed ablation of Sect.~\ref{sec:ablation}
inherits that limit. We claim no consistent improvement on the retention axis,
and did not fix in advance the margin a non-inferiority claim would need.
Finally, the method inherits whatever the frozen pipeline provides: it never
modifies training, so the error that remains when routing is correct and both
heads are consulted lies outside its reach.
